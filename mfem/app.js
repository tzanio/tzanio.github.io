...
let linuxReady=false,ready=false,currentPath='',dirty=false,sequence=0,serialBuffer='',terminalCwd=sourceRoot+'/examples',workspaceBusy=false,restoringWorkspace=false;
let editor,simulation,layout,offline,workspaces,developerTools,browserCompiler;
...
function rpc(op,values={},onProgress) {
  return new Promise((resolve,reject)=>{
    if(!linuxReady){reject(new Error('Linux is still starting; you can save once it is ready.'));return}
    const id=++sequence;
...
workspaces=WorkbenchWorkspaces.mount({rpc,journal:vm,readFile:path=>vm.read_file(path.replace(/^\//,'')),upload,editor,
...
browserCompiler=WorkbenchBrowserCompiler.mount({rpc,readFile:path=>vm.read_file(path.replace(/^\//,'')),upload,notice});
developerTools=WorkbenchDeveloperTools.mount({rpc,readFile:path=>vm.read_file(path.replace(/^\//,'')),editor,terminal,notice,
  openFile,isReady:()=>ready,getCwd:()=>terminalCwd,getRuntime:()=>({mfem:'4.10',mode:vm.mode,memoryMiB:768,userAgent:navigator.userAgent}),
});
...
$('backup').onclick=async()=>{
  ...
  try {
    ...
    path=await rpc('export',{written_paths:journal?.paths||[]},archiveProgress(activity));
    activity.update({stage:'download',label:'Preparing the browser download…'});
    const bytes=await vm.read_file(path.slice(1));
    WorkbenchOperations.download(activity,bytes,'mfem-workspace-'+new Date().toISOString().slice(0,10)+'.tar.gz');notice('Workspace archive ready. Download it again from Activity.');
...
async function importArchive(file,confirmed=false) {
  ...
  try {
    ...
    if(!confirmed&&!confirm('Import restores workspace files, including recorded deletions. Older archives overlay existing files. Unsaved editor tabs are kept. Continue?'))return;
    ...
    const journal=await vm.getWorkspaceWrites?.();
    await rpc('restore',{upload:uploaded,written_paths:journal?.paths||[]},archiveProgress(activity));uploaded=null;restoringWorkspace=false;
