...
(function () {
  ...
  function create(host, {rpc, readFile, upload, notice = console.warn, diagnostics = {}}) {
    ...
    function updateViewStatus(view) {
      ...
      if (!state) {label.textContent = 'Listening on localhost:19916';return}
    ...
    function initializeViewer(view) {
      ...
      const viewer = new glvis.State(view.visual, Math.max(1,view.visual.clientWidth),Math.max(1,view.visual.clientHeight));
    ...
    function addView() {
      ...
      if (!view) {
        ...
        if (view.viewer) view.viewer.emglv_.then(module => startRenderLoop(view,module));
      }
      ...
      resizeRequest = requestAnimationFrame(() => {
        ...
        for (const view of activeViews()) {
          ...
          if (view.viewer && (width !== view.viewer.logical_width_ || height !== view.viewer.logical_height_)) {
            enqueueView(view,() => view.viewer.setSize(width,height)).catch(report);
          }
        }
      });
    }
    ...
    addView();updateRecording();
    return {
      handle,resize,streams,recording,
